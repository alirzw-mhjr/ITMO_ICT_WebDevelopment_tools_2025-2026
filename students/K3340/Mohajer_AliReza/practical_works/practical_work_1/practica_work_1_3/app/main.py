...
async def lifespan(app: FastAPI):
    ...
    init_db()
    print(
        f"База данных подключена: {os.getenv('DATABASE_URL', 'sqlite:///./bookcrossing.db')}"
    )
    yield
    # При остановке
...
def root():
    return {
        "message": "Welcome to BookCrossing API!",
        "docs": "/docs",
        "environment": (
            "development" if "sqlite" in os.getenv("DATABASE_URL", "") else "production"
        ),
    }
...
def create_genre(
    genre_data: schemas.GenreCreate, session: Session = Depends(get_session)
):
    """Создать новый жанр"""
...
def create_book(book_data: schemas.BookCreate, session: Session = Depends(get_session)):
...
def update_book_genres(
    book_id: int, genre_ids: List[int], session: Session = Depends(get_session)
):
